...
if Options.Constrained.SharedParameters == "Full"
    ...
    [a, fval, exitflag] = fminsearch(@myFirstFit, a, opts);
    count = 1;
    while exitflag == 0
        [a, fval, exitflag] = fminsearch(@myFirstFit, a, opts);
    ...
    while 1
        ...
        for iF = 1:length(Options.InitialFactor)
            [a_test{iF}, f_test(iF)] = addAPeak_Full(a, Options.PointsPerPeaks, Options.InitialFactor(iF), Options.LoopMe, MinVar);
        end
        nPeak = nPeak+1;
        [~, Imin] = min(f_test);
        a = a_test{Imin};
    ...
    [a, fval, exitflag] = fminsearch(@mySecondFit, a, opts);
    count = 1;
    while exitflag == 0
        [a, fval, exitflag] = fminsearch(@mySecondFit, a, opts);
        if count > Options.LoopMe, break; else, count = count +1; end
    ...
    while 1
        ...
        a_test = {}; f_test = [];
        for iF = 1:length(Options.InitialFactor)
            [a_test{iF}, f_test(iF)] = addAPeak_Partial(a, Options.PointsPerPeaks, Options.InitialFactor(iF), Options.LoopMe, MinVar);
        end
        nPeak = nPeak+1;
        [~, Imin] = min(f_test);
        a = a_test{Imin};
